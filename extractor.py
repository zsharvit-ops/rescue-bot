...
def format_transcript_with_speakers(transcript: str) -> str:
    ...
    prompt = """אתה עורך תמלול מקצועי של שיחת חילוץ והצלה. קרא את השיחה הבאה וחלק אותה לפי דוברים.

חוקים:
1. זהה כמה דוברים יש (בדרך כלל 2: קצין חילוץ/מתשאל, ומדווח/נפגע/בן משפחה).
2. חפש שמות שנאמרים בשיחה — "שמי...", "אני...", "קוראים לי..." — השתמש בהם.
3. אם קצין מציג את עצמו — השתמש בשמו. אם לא — "קצין חילוץ".
4. אם המדווח מציג את עצמו — השתמש בשמו. אם לא — "מדווח".
5. פרמט מחייב: שם_הדובר: הטקסט שאמר
6. כל שינוי דובר — שורה חדשה בלבד (ללא שורה ריקה מיותרת).
7. שמור על כל המידע במדויק — אל תשמיט שום דבר ואל תשנה מילים.
8. החזר את התמלול המעוצב בלבד, ללא הסברים או כותרות.
